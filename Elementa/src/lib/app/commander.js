...
const handleRecordClick = async (e) => {
  ...
  if (isRecordingActive()) {
    ...
    // Show success message if we got text
    if (transcript) {
      showStatusMessage(page, "Voice transcription complete!", "success", {
        autoHide: true,
        duration: 2500,
      });
    }
    ...
    try {
      // Start recording with callback to update textarea in real-time
      await startRecording(element, (transcriptText, isFinal) => {
        // Append transcribed text to existing content
        const existingText = textarea.dataset.originalText || "";
        textarea.value = existingText + transcriptText;

        // Keep cursor at end
        textarea.scrollTop = textarea.scrollHeight;
      });

      // Store original text so we can append to it
      textarea.dataset.originalText = textarea.value;

      updateRecordButton(btn, true);
      currentRecordingButton = btn;
      showStatusMessage(page, "Listening... Speak now.", "info", {
        autoHide: true,
        duration: 3000,
      });
    } catch (err) {
...
const handleElementPanelToggle = () => {
  ...
  // If opening element panel, close sidebar first
  if (!isOpen) {
    const sidebar = document.querySelector(".sidebar");
...
// Track confirmation state for two-click delete pattern
...
export const handleEntryControlClick = async (e) => {
  ...
  switch (action) {
    case "open-move-menu":
      ...
      // Show folder list is already visible - no alert needed
      break;

    case "edit":
      // Stub - show inline message
      const editBtn = icon;
      editBtn.textContent = "Coming soon";
      setTimeout(() => {
        editBtn.textContent = "";
    ...
    case "delete":
      const deleteBtn = icon;

      // Check if already in confirmation state
      if (pendingDeletes.has(entryId)) {
        ...
        pendingDeletes.set(entryId, true);
        deleteBtn.textContent = "?";
...
export const initApp = async () => {
  ...
  allElements.forEach((element) => {
    const navItem = document.querySelector(
      `.nav-bar .nav-item[data-element="${element}"]`
    );
    ...
    const headerContainer = document.querySelector(
      `.page .header-icon[data-element="${element}"]`
    );
    ...
    const promptEl = document.querySelector(
      `.prompt-text[data-prompts-id="${element}"]`
    );
    if (promptEl) {
  ...
  const elementPanelBackdrop = document.querySelector(
  ...
  if (elementPanelClose) {
    elementPanelClose.addEventListener("click", () =>
      toggleElementPanel(false)
    );
  }
  if (elementPanelBackdrop) {
    elementPanelBackdrop.addEventListener("click", () =>
      toggleElementPanel(false)
    );
  }
